...
if isempty(map), %monochrome images return empty colormap (JDY)
    map = [0,0,0;1,1,1];
end
colormap(map)
...
switch XLimButton,
     case 'OK',
	  drawnow
	  [XAxisXpixels,XAxisYpixels] = ginputz;
...
switch YLimButton,
     case 'OK',
	  drawnow
	  [YAxisXpixels,YAxisYpixels] = ginputz;
...
% %%%%%%%%%%%%%% DATA ACQUISITION LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     if nargout  == 0,
	  ...
	  if isequal([writefname, writepname],[0,0]), %JDY
	       close(FigName)
	       break
	       return
	  end
...
% helper function ginputz (JDY)
% implements a zoom feature
function [x,y,bn] = ginputz()
[x,y,bn] = ginput(1);
while bn == double('z'),
    zoom on
    set(gcf,'MenuBar','figure')
    while ~isequal(get(gcf,'CurrentCharacter'),'x'), 
        waitforbuttonpress
    end
